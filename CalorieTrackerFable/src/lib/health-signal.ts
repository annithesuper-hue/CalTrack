...
function buildHealthLevelMeta(colors: ColorPalette): Record<HealthLevel, { color: string; soft: string }> {
  return {
    ...
    red: { color: colors.danger, soft: colors.dangerSoft },
  };
}

/** Theme-aware health-level metadata — call inside a component so colors update with the theme. */
export function useHealthLevelMeta(): Record<HealthLevel, { color: string; soft: string }> {
  const { colors } = useTheme();
  return buildHealthLevelMeta(colors);
}
