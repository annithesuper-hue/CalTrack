...
export type Meal = MacroSet & {
  ...
  emoji: string;
  mealType: MealType;
  /** Dietary fiber in grams. Defaults to 0 for meals logged before fiber tracking existed. */
  fiber: number;
...
export type AnalysisResult = MacroSet & {
  ...
  items: AnalysisItem[];
  /** Dietary fiber in grams. Optional because the AI photo-analysis path doesn't estimate it (only food-database lookups do); treated as 0 when absent. */
  fiber?: number;
