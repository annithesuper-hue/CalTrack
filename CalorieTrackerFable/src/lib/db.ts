...
function migrateMealType(): void {
  const cols = db.getAllSync<{ name: string }>('PRAGMA table_info(meals)');
  if (cols.some((c) => c.name === 'meal_type')) return;
  db.execSync(`ALTER TABLE meals ADD COLUMN meal_type TEXT NOT NULL DEFAULT 'other';`);
}
...
export function insertMeal(meal: Meal): void {
  db.runSync(
    `INSERT INTO meals (id, name, emoji, calories, protein, carbs, fat, photo_uri, note, meal_type, day, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    meal.id,
    meal.name,
    meal.emoji,
...
export function updateMeal(meal: Meal): void {
  db.runSync(
    `UPDATE meals SET name = ?, emoji = ?, calories = ?, protein = ?, carbs = ?, fat = ?, note = ?, meal_type = ?
     WHERE id = ?`,
    meal.name,
    meal.emoji,
    Math.round(meal.calories),
