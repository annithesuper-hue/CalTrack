import { ApiError, requestJson, sanitizeApiKey } from './api-client';
 ...
 */
const GEMINI_API_KEYS = [sanitizeApiKey(process.env.EXPO_PUBLIC_GEMINI_API_KEY), sanitizeApiKey(process.env.EXPO_PUBLIC_GEMINI_API_KEY_2)].filter(
  (k): k is string => k !== null,
);

/**
 * Non-secret diagnostic (never includes key values) — safe to show in the
 * UI so this exact "missing or invalid" failure can be debugged from a real
 * device/TestFlight build without a debugger attached. See Settings.
 */
export function geminiKeyDiagnostics(): { key1: boolean; key2: boolean; count: number } {
  return {
    key1: sanitizeApiKey(process.env.EXPO_PUBLIC_GEMINI_API_KEY) !== null,
    key2: sanitizeApiKey(process.env.EXPO_PUBLIC_GEMINI_API_KEY_2) !== null,
    count: GEMINI_API_KEYS.length,
  };
}
