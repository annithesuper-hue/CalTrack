...
const searchCache = new Map<string, { at: number; results: FoodResult[] }>();
const CACHE_TTL_MS = 5 * 60 * 1000;
const CACHE_MAX_ENTRIES = 50;

function cacheGet(key: string): FoodResult[] | null {
...
function cacheSet(key: string, results: FoodResult[]): void {
 ...
 * rather than failing the whole search. Repeat queries within a few minutes
 * are served from an in-memory cache instead of hitting the network again.
 */
export async function searchAllFoods(query: string, signal?: AbortSignal): Promise<FoodResult[]> {
  ...
  const merged = interleave([generic, branded]);
  // Only cache successful, reasonably complete results — not a partial
  // result set from a source that failed, so a flaky network doesn't get
  // "stuck" in the cache for the TTL window.
  if (usdaSettled.status === 'fulfilled') cacheSet(cacheKey, merged);
  return merged;
}
...
/**
 * Converts a FoodResult + quantity (in the result's unit, e.g. grams) into
 * a loggable AnalysisResult. Multiplication happens against the food's
 * precise per-gram values; rounding to whole units happens only here, at
 * the boundary where the number becomes a user-editable/display value —
 * every calculation upstream (perGram derivation in usda.ts/openfoodfacts.ts)
 * keeps full float precision so small quantities don't lose accuracy.
 */
export function foodResultToAnalysis(food: FoodResult, quantity: number): AnalysisResult {
  ...
  return {
    ...
    fiber: Math.max(0, Math.round(food.perGram.fiber * q)),
    confidence: 'high',
    items: [],
  };
}
