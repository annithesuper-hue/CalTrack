...
import { useMealTypeMeta } from '@/lib/meal-type';
...
export function MealCard({ meal, onPress }: { meal: Meal; onPress?: () => void }) {
  ...
  const { macroMeta } = theme;
  const styles = useMemo(() => createStyles(theme), [theme]);
  const mealTypeMeta = useMealTypeMeta()[meal.mealType];
  return (
    <Pressable
      onPress={() => {
        if (onPress) {
      ...
      style={({ pressed }) => [styles.card, pressed && { backgroundColor: theme.colors.cardPressed }]}>
      {meal.photoUri ? (
