      ...
      <View onLayout={(e) => setWidth(e.nativeEvent.layout.width)}>
        {width > 0 && (
          <Svg width={width} height={PLOT_HEIGHT}>
            {/* baseline */}
            <Line x1={0} y1={PLOT_HEIGHT - 0.5} x2={width} y2={PLOT_HEIGHT - 0.5} stroke={theme.colors.hairline} strokeWidth={1} />
            {data.map((d, i) => {
                ...
                <React.Fragment key={d.day}>
                  {h > 0 && (
                    <Rect
                      ...
                      height={h + BAR_RADIUS}
                      rx={BAR_RADIUS}
                      fill={isToday ? theme.colors.ink : theme.colors.green}
                      opacity={isToday ? 1 : 0.85}
                    />
                  )}
                  {/* clip the rounded bottom back to the baseline */}
                  {h > 0 && <Rect x={x} y={PLOT_HEIGHT} width={barW} height={BAR_RADIUS} fill={theme.colors.card} />}
            ...
            {/* goal reference line */}
            <Line x1={0} y1={goalY} x2={width} y2={goalY} stroke={theme.colors.inkSecondary} strokeWidth={1.5} strokeDasharray="5,4" />
            <SvgText x={width - 2} y={goalY - 5} fontSize={10} fontWeight="600" fill={theme.colors.inkSecondary} textAnchor="end">
        ...
        {width > 0 && (
          <Svg width={width} height={PLOT_HEIGHT}>
            ...
            {data.map((d, i) => {
              const x = i * (barW + gap);
              const segments = [
                { value: d.protein, color: theme.macroMeta.protein.color },
                { value: d.carbs, color: theme.macroMeta.carbs.color },
                { value: d.fat, color: theme.macroMeta.fat.color },
              ];
              let y = PLOT_HEIGHT;
              const isToday = d.day === today;
              return (
  ...
  return (
    <View style={styles.axisRow}>
      {data.map((d) => (
        <Text
          key={d.day}
          style={[styles.axisLabel, d.day === today && { color: theme.colors.ink, fontWeight: '700' }]}>
          {weekdayLetter(d.day)}
        </Text>
      ))}
    </View>
