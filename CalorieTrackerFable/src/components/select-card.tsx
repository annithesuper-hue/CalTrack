  ...
  return (
    <Pressable
      ...
      <View style={styles.textWrap}>
        <Text style={[styles.title, selected && { color: colors.onAccent }]}>{title}</Text>
        {subtitle ? (
          <Text style={[styles.subtitle, selected && { color: colors.onAccent, opacity: 0.7 }]}>{subtitle}</Text>
        ) : null}
      </View>
      {selected ? <SymbolView name="checkmark.circle.fill" size={22} tintColor={colors.onAccent} /> : null}
    </Pressable>
  );
}
